...
const { ModelPreparacion, ModelReceta, ModelCocinero, ModelCopia } = require('../models');
...
class ControllersPreparacion {
    ...
    //Para copiar la preparación al Seguimiento
    async copyPreparacion(req,res=response){
        try {
            const PreparacionesCopys = await ModelPreparacion.aggregate([{ $match: { replicated:false } }]);
            const PreparacionesUpdate= await ModelPreparacion.updateMany({ replicated:false }, { $set: { replicated:true} })
            const Copys = await ModelCopia.insertMany(PreparacionesCopys);
            res.json({
                Data:Copys
            })
        } catch (error) {
